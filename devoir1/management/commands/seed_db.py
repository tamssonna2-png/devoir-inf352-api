...
from devoir1.models import Utilisateur, Controleur  # Ajuste les imports selon tes modèles réels

class Command(BaseCommand):
    help = "Peuple la base de données Neon avec des données de test uniques sans doublons"

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("Début du peuplement de la base de données..."))

        # 1. Génération d'utilisateurs uniques
        # On utilise un suffixe UUID court pour garantir l'unicité à chaque lancement du script
        unique_suffix = str(uuid.uuid4())[:8]
        
        utilisateurs_data = [
            {"nom": f"Sonna Clautel {unique_suffix}", "mot_de_passe": "Securise123!", "solde": 25000.00},
            {"nom": f"Jean Kane {unique_suffix}", "mot_de_passe": "PassBancaire456!", "solde": 10000.00},
            {"nom": f"Marie Noubissie {unique_suffix}", "mot_de_passe": "TopSecret789!", "solde": 50000.00},
        ]

        for u_data in utilisateurs_data:
            # get_or_create cherche si le 'nom' existe déjà. 
            # S'il existe, il le récupère. Sinon, il le crée avec les 'defaults'.
            utilisateur, created = Utilisateur.objects.get_or_create(
                nom=u_data["nom"],
                defaults={
                    "mot_de_passe": u_data["mot_de_passe"],
                    "solde": u_data["solde"]
                }
            )
            
            if created:
                self.stdout.write(f"✅ Utilisateur créé : {utilisateur.nom} (Solde: {utilisateur.solde})")
            else:
                self.stdout.write(self.style.WARNING(f"⚠️ L'utilisateur {utilisateur.nom} existait déjà. Passé."))

        # 2. Exemple pour un Contrôleur (si tu as ce modèle)
        # Tu peux appliquer la même logique get_or_create
        try:
            controleur, created = Controleur.objects.get_or_create(
                nom=f"Admin_Controleur_{unique_suffix}",
                defaults={"statut": "ACTIVE"}
            )
            if created:
                self.stdout.write(f"✅ Contrôleur créé : {controleur.nom}")
        except NameError:
            pass  # Passe si le modèle Controleur n'est pas configuré ainsi
